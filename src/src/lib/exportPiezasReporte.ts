...
// agrupado por tipo igual que la vista de la página Piezas, con las
...
import { agruparPorTipo } from './piezasGrupos'
...
export async function exportPiezasReporteToPdf(piezas: Pieza[]) {
  ...
  const grupos = agruparPorTipo(piezas)

  for (const { tipo, items } of grupos) {
    ensureSpace(16)
    doc.setFontSize(12)
    doc.setFont('helvetica', 'bold')
    doc.text(`${tipo} (${items.length})`, margin, y)
